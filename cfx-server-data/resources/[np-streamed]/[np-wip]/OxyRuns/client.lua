...
Citizen.CreateThread(function()
	while doingOxy do
		if not atOxy then
			TriggerEvent("np-polyzone:enter", "AtOxy")
			Wait(500)
		else
			TriggerEvent("np-polyzone:exit", "AtOxy")
			Wait(500)
		end
	end
...
RegisterNetEvent('oxyrun:client:sendToOxy', function()
    ...
    TriggerClientEvent('DoLongHudText', target, "Get a vehicle, Make sure its stolen", 5)
	gettingBox = true
	gettingVehicle = true
end)
...
AddEventHandler('onClientResourceStart', function (resourceName)
	...
	for route = 1, #Config.Routes do
		...
		exports['np-polyzone']:AddBoxZone("AtOxy", position, 15, 15, {
			name="AtOxy",
			heading=335.31,
			minZ=position.z-10,
			maxZ=position.z+5,
