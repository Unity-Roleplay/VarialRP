...
function StartPeekin()
    ...
    Citizen.CreateThread(function()
        ...
        while IsPeeking do
            ...
            if LocalPlayer.state.disabled or LocalPlayer.state.dead or IsPedInAnyVehicle(PlayerPedId()) or IsPlayerFreeAiming(PlayerId()) then 
                StopPeekin()
                return 
            end
...
RegisterNetEvent("np-target:changed", function(pTarget, pEntityType, pEntityOffset)
...
RegisterNetEvent('np-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
RegisterNetEvent('np-polyzone:exit', function(zoneName)
    if not PeekEntries['polytarget'][zoneName] then return end
