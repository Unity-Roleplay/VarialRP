local copCars = {
    ...
    "npolexp", -- Explorer
    "npolchar", -- Charger
    "npolmm", -- Interceptor Bike
    "npolchal", -- Interceptor Challenger
    "npolvette", -- Interceptor Corvette
    "npolstang", -- Interceptor Mustang
    "npolblazer", -- Ranger Blazer
    "npolcoach", -- Prison Bus
    "polas350", -- Helicopter Helicopter
    "bcat", -- Yorkcat
    "polfegway", -- Creampi Shitmobile
    "predator" -- Boat
}
...
function toggleHarness()
   ...
   -- disableTurning()
    ...
    if harness then
        ...
        finished = exports["np-taskbar"]:taskBar(3000,"Putting on Harness",true)
    end
    if (finished == 100) then
        if playerPed == driverPed then
            if not harness then
                TriggerEvent("harness", true, harnessDurability)
                TriggerEvent('hud:client:harness')
