    ...
    Citizen.CreateThread(function()
        ...
        while CurrentlyCornering do
            ...
            if not foundPed then
                ...
                RPC.execute("np-weed:cornerPed", coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
            end
...
AddEventHandler("np-weed:cornerSale", function(pContext, pEntity)
    ...
    local baggies = exports["np-inventory"]:getItemsOfType("weedbaggie", 1, true)
    if not baggies or #baggies == 0 then
        TriggerEvent("DoLongHudText", _L('weed-corner-missing-baggies', 'You need more baggies to sell'))
        return
    end

    local baggie = baggies[1]
    local baggieInfo = json.decode(baggie.information)
    ...
    local didSale = RPC.execute("np-weed:cornerSale", CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone, baggieInfo)
    if didSale then
        PlayAmbientSpeech1(pEntity, "Chat_State", "Speech_Params_Force")
        TriggerEvent("money:clean", 0.035)
...
AddEventHandler("np-weed:prepareBaggies", function(pContext, pEntity)
    if CornerConfig.RequireScales then
        local hasScale = exports["np-inventory"]:hasEnoughOfItem("qualityscales", 1, false) or exports["np-inventory"]:hasEnoughOfItem("smallscales", 1, false)
    ...
    local finished = exports["np-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        ...
        RPC.execute("np-weed:prepareBaggies", info)
    end
end)
