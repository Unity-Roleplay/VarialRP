...
Citizen.CreateThread(function()
    ...
    IsWeedProcessing = RPC.execute("np-weed:currentlyProcessing")
    IsWeedProcessingFinished = RPC.execute("np-weed:readyForPickup")
end)
...
AddEventHandler("np-weed:deliver_package", function()
    ...
    PlayAmbientSpeech1(CurrentDeliveryNPC.entity, "Chat_State", "Speech_Params_Force")
    local finished = exports["np-taskbar"]:taskBar(15000, _L('weed-taco-count-bills', 'Counting Bills'), false, true, false, false, nil, 5.0, PlayerPedId())
...
AddEventHandler("np-weed:purchase_list", function()
    ...
    context[#context+1] = {
        ...
        key = {},
        action = 'np-weed:buyDeliveryList',
    }
    context[#context+1] = {
        title = 'Cancel',
        key = { id = pPlantId },
        action = 'np-weed:closeContext',
    }
...
AddEventHandler("np-weed:package_goods", function()
    ...
    local finished = exports["np-taskbar"]:taskBar(10000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, PlayerPedId())
    if finished == 100 then
