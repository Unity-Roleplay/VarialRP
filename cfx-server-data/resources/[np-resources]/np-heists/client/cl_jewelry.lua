...
function uhavewapon2()
    ...
    if weaponTypeC() > 1 then
        ...
        TriggerEvent("DoLongHudText", "You don't have required power for breaking glass" , 2)
    end
end
...
AddEventHandler("pacific:ptfxparticle", function(method)
    ...
    while not HasNamedPtfxAssetLoaded("scr_ornate_heist") do
        ...
        ptfx = vector3(-596.27, -283.81, 50.32)
...
local thermitepos = vector3(-596.0, -282.80, 50.3236)
local ped = PlayerPedId()
local playercoords = GetEntityCoords(ped)
...
if #(playercoords - thermitepos) < 2.0 then
	...
	TaskGoStraightToCoord(PlayerPedId(), -596.39, -283.86, 50.32, 1.0, -1, 296.8, 0.0)
	Citizen.Wait(4500)
	if exports["isPed"]:isPed("countpolice") >= 1 then
	    exports["np-memory"]:thermiteminigame(10, 3, 3, 10,
			...
			TriggerEvent("DoLongHudText", "Already opened!" , 2)
		end    
else
	print('notdistaance')
end
...
AddEventHandler('dark-jewelry:jewelry:SmashFunctions1', function(p1, p2, pContext)
	if notsmashed1 then
		...
		TriggerEvent("DoLongHudText", "Smashed Already" , 2)
	end
end
else
	TriggerEvent("DoLongHudText", "Smashed Already" , 2)
...
AddEventHandler('dark-jewelry:jewelry:cooldown', function()
	TriggerEvent("DoLongHudText", "In 15 minutes, doors will be closed!!" , 2)
	Wait(900000)
