...
$(document).ready(() => {
    ...
    $(".nav-item").click(function () {
        if ($(this).hasClass("active-nav") == false) {
            ...
            // if ($(this).hasClass("staffmanagement-nav-item") == true) {
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-create", function () {
        let tempalte = "LSPD Report\n\nDate:    Time:\n\nReporting Officer: [Name Here]\n\nAssisting Officer/s: [Name Here]\n\nSuspect/s: [Name Here]\n\nWitnesses: [Name Here]\n\nLocation:\n\nDebrief: "
    ...
    $(".sm-items").on("click", ".sm-item", function () {
        $.post('https://np-mdt/getStaffData', JSON.stringify({
            id: $(this).data("id")
        }));
    });

    $(".staff-information-hire").click(function () {
        $(".hirecid-container").fadeIn(0)
        $(".hirecid-inner-container").slideDown(500)
        $(".hirecid-inner-container").fadeIn(500)
        $(".hirecid-input").val("")
    });
    $(".staff-information-dismiss").click(function () {
        $(".dismiss-container").fadeIn(0)
        $(".dismiss-inner-container").slideDown(500)
        $(".dismiss-inner-container").fadeIn(500)
        $(".dismiss-container").data("id", selectedStaffMember["id"])
        $(".dismiss-input").val(selectedStaffMember["first_name"] + " " + selectedStaffMember["last_name"])
    });
    ...
    $(".staff-information-title-holder").on("click", ".staff-information-save", function () {
        if (canSaveVehicle) {
            canSaveVehicle = false
            $(".staff-information-save").empty();
            $('.staff-information-save').prepend(`<span class="fas fa-check"></span>`);
            setTimeout(() => {
                $(".staff-information-save").empty();
                $(".staff-information-save").html("Save");
                canSaveVehicle = true
            }, 750);
            setTimeout(() => {
                let dbid = selectedStaffMember["id"]
                let callsign = $(".staff-info-callsign-input").val()
                let dept = $(".staff-info-department-input").val()
                let rank = $(".staff-info-rank-input").val()
                let nickname = $(".staff-info-nickname-input").val()
                let badgeImage = $(".staff-info-badgeimage-input").val()

                $.post('https://np-mdt/saveStaffInfo', JSON.stringify({
                    dbid: dbid,
                    callsign: callsign,
                    dept: dept,
                    rank: rank,
    ...
    $(".hirecid-buttons").on("click", ".hirecid-cancel", function () {
        $(".hirecid-inner-container").slideUp(500)
        $(".hirecid-inner-container").fadeOut(500)
        setTimeout(() => {
            $(".hirecid-container").slideUp(500)
            $(".hirecid-container").fadeOut(500)
            $(".hirecid-input").val("")
        }, 500)
    })

    $(".hirecid-buttons").on("click", ".hirecid-submit", function () {
        const hirecid = $(".hirecid-input").val()
        $.post('https://np-mdt/hirecid', JSON.stringify({
            cid: hirecid
        }));
        $.post('https://np-mdt/getAllStaff', JSON.stringify({}));
    ...
    $(".dismiss-buttons").on("click", ".dismiss-cancel", function () {
        $(".dismiss-inner-container").slideUp(500)
        $(".dismiss-inner-container").fadeOut(500)
        setTimeout(() => {
            $(".dismiss-container").slideUp(500)
            $(".dismiss-container").fadeOut(500)
            $(".dismiss-input").val("")
        }, 500)
    })

    $(".dismiss-buttons").on("click", ".dismiss-submit", function () {
        const dismiss = $(".dismiss-input").val()
        $.post('https://np-mdt/dismissStaff', JSON.stringify({
            cid: selectedStaffMember["id"]
        }));
        $.post('https://np-mdt/getAllStaff', JSON.stringify({}));
    ...
    function JobColors(sentJob, sentDepartment) {
        if (sentJob) {
            if (sentJob == 'police') {
                if (sentDepartment == '1') { //BCSO
                ...
                else if (sentDepartment == '3') { //SAPR
                    document.documentElement.style.setProperty('--color-1', '#1e5530');
                    document.documentElement.style.setProperty('--color-2', '#215f35');
                    document.documentElement.style.setProperty('--color-3', '#2C7B46');
                    document.documentElement.style.setProperty('--color-4', '#235e38');
                    document.documentElement.style.setProperty('--color-5', '#153821');
                    document.documentElement.style.setProperty('--color-6', '#121f2c');
                    document.documentElement.style.setProperty('--color-7', '#1C522F');
                    document.documentElement.style.setProperty('--color-8', '#2554cc');
                    document.documentElement.style.setProperty('--color-9', '#6e7c73');
                    $(".badge-logo").attr('src','img/ranger_badge.png');
                    $(".header-title").html("SAN ANDREAS STATE PARK RANGERS");
                }
                else { //LSPD
                    document.documentElement.style.setProperty('--color-1', '#1E3955');
                    document.documentElement.style.setProperty('--color-2', '#213F5F');
                ...
                $(".roster-iframe").attr("src", "https://docs.google.com/spreadsheets/d/1vhysIDlZr5Y27NKBvy-BE7WOAt0zwWCkNQ7cV2FUrkg/")                
            } else if (sentJob == 'ems') {
                ...
                $(".roster-iframe").attr("src", "https://docs.google.com/spreadsheets/d/1vhysIDlZr5Y27NKBvy-BE7WOAt0zwWCkNQ7cV2FUrkg/pubhtml?widget=true&amp;headers=false")
            }
        }
    }
    ...
    window.addEventListener('message', function (event) {
        ...
        if (e.type == "show") {
            ...
            MyName = e.fullname
            if (e.rankNo >= 7) {
                $(".stafflogs-nav-item").css("display", "flex");
                $(".staffmanagement-nav-item").css("display", "flex");
            }
            ...
            if (value) {
                ...
                let DispatchItem = `<div class="active-calls-item" data-id="${value.eventId}" data-x="${value.origin.x}" data-y="${value.origin.y}" data-canrespond="false"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#</div><div class="call-code priority-${value.priority}">${value.dispatchCode}</div><div class="call-title">${value.dispatchMessage}</div><div class="call-radio"></div></div><div class="call-item-bottom">`
                //${value.ctxId}
                if (value.dispatchCode == "911" || value.dispatchCode == "311" || value.dispatchCode == "10-13A") {
                    DispatchItem = `<div class="active-calls-item" data-id="${value.eventId}"  data-x="${value.origin.x}" data-y="${value.origin.y}" data-canrespond="true"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#</div><div class="call-code priority-${value.priority}">${value.dispatchCode}</div><div class="call-title">${value.dispatchMessage}</div><div class="call-radio"></div></div><div class="call-item-bottom">`
                }
        ...
        } else if (e.type == "getAllStaff") {
            ranks = e.ranks
            let table = e.data
            $(".sm-items").empty();
            $.each(table, function (index, value) {
                let rankVal = value.rank
                let deptVal = value.dept
                // if (value.rank !== null && value.dept !== null) {
                //     console.log(JSON.stringify(ranks[value.dept.toString()]))
                //     console.log(JSON.stringify(ranks[value.dept.toString()][value.rank.toString()]))
                //     rankVal = ranks[value.dept][value.rank]
                // }               
                            
                $('.sm-items').prepend(`
                <div class="sm-item sm-background-${value.dept}" data-id="${value.id}">
                    <div style="display: flex; flex-direction: column; margin-top: 2.5px; margin-left: 5px; width: 100%; padding: 5px;">
                    <div style="display: flex; flex-direction: column;">
                        <div class="sm-item-title">${value.first_name} ${value.last_name}</div>
                            <div class="sm-tags">
                                <div class="sm-tag green-tag">Job: ${value.job}</div>
                                <div class="sm-tag green-tag">Rank: ${rankVal}</div>
                                <div class="sm-tag green-tag">Nickname: ${value.nickname}</div>
                                <div class="sm-tag green-tag">Callsign: ${value.callsign}</div>
                                <div class="sm-tag green-tag">Department: ${deptVal}</div>
                            </div>
                        </div>
                        <div class="sm-bottom-info">
                            <div class="sm-id">ID: ${value.id} · Owner: ${value.cid}</div>
                        </div>
                    </div>
                </div>
                `)
            })
        } else if (e.type == "getStaffInfo") {
            selectedStaffMember = e.data
            $('.staff-info-name-input').val(selectedStaffMember["first_name"] + " " + selectedStaffMember["last_name"])
            $('.staff-info-callsign-input').val(selectedStaffMember["callsign"])

            $(".staff-info-department-input").empty();
            $.each(ranks, function (index, value) {
                stringVal = "<option value=" + index

                if (index == selectedStaffMember["dept"]) {
                    stringVal = stringVal + " selected"

                    $(".staff-info-rank-input").empty();
                    $.each(value, function(index2, value2) {
                        if (index2 !== "Name") {
                            stringVal2 = "<option value=" + index2
                            if (index2 == selectedStaffMember["rank"]) {
                                stringVal2 = stringVal2 + " selected"   
                            }
                            stringVal2 = stringVal2 + ">" + value2 + "</option>"
                            $('.staff-info-rank-input').prepend(stringVal2)
                        }
                    })
                }
                stringVal = stringVal + ">" + value["Name"] + "</option>"
                $('.staff-info-department-input').prepend(stringVal)
            })

            $('.staff-info-nickname-input').val(selectedStaffMember["nickname"])
            $('.staff-info-badgeimage-input').val(selectedStaffMember["badgeImage"])
            ...
            if (sapr.length > 0) {
                $.each(sapr, function (index, value) {
                    ...
                    <div class="active-unit-item" data-id="${value.cid}">
                        <div class="unit-status ${statuscolor}">${status}</div>
                        <div class="unit-job active-info-job-sapr">SASPR</div>
                        <div class="unit-name">(${callsign}) ${value.name}</div>
                        <div class="unit-radio">${radio}</div>
                    </div>
                    `)
...
function fidgetSpinner(page) {
    ...
    if (page == '.staffmanagement-page-container') {
        $.post('https://np-mdt/getAllStaff', JSON.stringify({}));
    }
