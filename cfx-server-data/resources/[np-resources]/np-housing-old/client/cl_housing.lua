...
AddEventHandler('housing:info:realtor', function(args)
	...
	elseif args == "modify" then
		...
		if modifying then
			...
			modifying = true
			print('This is my house model', house_model)
			TriggerServerEvent("housing:requestSpawnTable",modifyingTable, house_id, modifyingTable["info"], house_model)
...
Citizen.CreateThread(function()
    while true do
        ...
        if insidehouse == true then
            ...
            --         -- exports["np-ui"]:showInteraction("Enter", "classic")
            ...
            if backdoorinside["x"] ~= 0.0 and #(vector3(backdoorinside["x"],backdoorinside["y"],backdoorinside["z"]-0.3) - plyCoords) < 1.5 then
                ...
                exports["np-ui"]:showInteraction("Leave", "classic")
                if IsControlJustReleased(2, 38) then
                    TriggerEvent("housing:exit:backdoor",backdooroutside["x"],backdooroutside["y"],backdooroutside["z"],backdooroutside["h"])
                    Citizen.Wait(3000)
                end
                -- else
                --     exports["np-ui"]:hideInteraction("classic")
                -- end
            

            

            elseif clothing["x"] ~= 0.0 and #(vector3(clothing["x"],clothing["y"],clothing["z"]-0.3) - plyCoords) < 1.5 then
                -- if #(vector3(clothing["x"],clothing["y"],clothing["z"]-0.3) - plyCoords) < 1.5 then
                TriggerEvent('np-clothing:OutfitsActive', true)
                    -- DrawText3Ds( clothing["x"],clothing["y"],clothing["z"] , '/outfits to change clothing' )
                exports["np-ui"]:showInteraction("Outfits", "classic")
            -- else
            --         TriggerEvent('np-clothing:OutfitsActive', false)
            --         exports["np-ui"]:hideInteraction("classic")
            --     end
            -- end
            

            elseif storage["x"] ~= 0.0 and #(vector3(storage["x"],storage["y"],storage["z"]-0.3) - plyCoords) < 1.5 then
                ...
                exports["np-ui"]:showInteraction("[E] Stash", "classic")
                if IsControlJustReleased(2, 38) then
                    TriggerEvent('InteractSound_CL:PlayOnOne','StashOpen', 0.6)
                    TriggerEvent("server-inventory-open", "1", "housing-"..house_poi["info"])
                end
            --     else
            --         exports["np-ui"]:hideInteraction("classic")
            --     end
 	...
 	while true do
		...
		if myhouseid == 0 then
			...
			if entryType == 1 then
				if myhouseid ~= 0 then
					if mansion then
						...
						if (#(vector3(curHouseCoords["x"]-5.5793,curHouseCoords["y"]+5.100,curHouseCoords["z"]-32.42) - playerCoords) < 1.2 ) then
							DrawText3Ds( curHouseCoords["x"]-5.5793,curHouseCoords["y"]+5.100,curHouseCoords["z"]-32.82, '~p~'..Controlkey["housingMain"][2]..'~s~ front door' )
						...
						if robberyExitCoordsMansions[myhouseid] then
							if robberyExitCoordsMansions[myhouseid]["x"] ~= 0.0 then
								if (#(vector3(curHouseCoords["x"]+14.1793,curHouseCoords["y"]+1.900,curHouseCoords["z"]-32.42) - playerCoords) < 1.2 ) then
									DrawText3Ds( curHouseCoords["x"]+14.1793,curHouseCoords["y"]+1.900,curHouseCoords["z"]-32.82, '~p~'..Controlkey["housingMain"][2]..'~s~ back door' )
									if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
					...
					elseif mansion2 then
						...
						if (#(vector3(curHouseCoords["x"]+7.5793,curHouseCoords["y"]+6.400,curHouseCoords["z"]-19.42) - playerCoords) < 1.2 ) then
							DrawText3Ds( curHouseCoords["x"]+7.5793,curHouseCoords["y"]+6.400,curHouseCoords["z"]-19.42, '~p~'..Controlkey["housingMain"][2]..'~s~ front door' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerServerEvent("houserobberies:exit",myhouseid,mansion,false)
								Citizen.Wait(5000)
							end
					...
					elseif shopshit then
						...
						if (#(vector3(curHouseCoords["x"] - 0.03, curHouseCoords["y"] - 7.86, curHouseCoords["z"] - 122) - playerCoords) < 1.2 ) then
							DrawText3Ds( curHouseCoords["x"] - 0.03, curHouseCoords["y"] - 7.86, curHouseCoords["z"] - 122, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif trailer then
						if (#(vector3((curHouseCoords["x"]-1.443),(curHouseCoords["y"]-1.892),(curHouseCoords["z"]-74.0)) - playerCoords) < 100.2 ) then
							DrawText3Ds( (curHouseCoords["x"]-1.443),(curHouseCoords["y"]-1.892),(curHouseCoords["z"]-74.0), '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
						...
						if (#(vector3(curHouseCoords["x"] + 4.3,curHouseCoords["y"] - 15.95,curHouseCoords["z"]-21.42) - playerCoords) < 1.2 ) then
							DrawText3Ds( (curHouseCoords["x"] + 4.3),(curHouseCoords["y"] - 15.95),(curHouseCoords["z"]-21.42), '~p~'..Controlkey["housingMain"][2]..'~s~ front door' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerServerEvent("houserobberies:exit",myhouseid,mansion,false)
								Citizen.Wait(5000)
							end
						...
						if robberyExitCoords[myhouseid] then
							if robberyExitCoords[myhouseid]["x"] ~= 0.0 then
								if (#(vector3(curHouseCoords["x"] -3.8,curHouseCoords["y"] + 5.25,curHouseCoords["z"]-21.42) - playerCoords) < 1.2 ) then
									DrawText3Ds( (curHouseCoords["x"] - 3.8),(curHouseCoords["y"] + 5.25),(curHouseCoords["z"]-21.42), '~p~'..Controlkey["housingMain"][2]..'~s~ back door' )
									if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
										TriggerServerEvent("houserobberies:exit",myhouseid,mansion,true)
										Citizen.Wait(5000)
									end
				...
				if mansion then
					if (#(vector3(curHouseCoords["x"]-5.5793,curHouseCoords["y"]+5.100,curHouseCoords["z"]-32.42) - playerCoords) < 1.2 ) then
						DrawText3Ds( curHouseCoords["x"]-5.5793,curHouseCoords["y"]+5.100,curHouseCoords["z"]-32.82, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
					...
					if (#(vector3(curHouseCoords["x"]-1.4,curHouseCoords["y"]-1.47,curHouseCoords["z"]-32.2) - playerCoords) < 2.2 ) then
						...
						if ( not rentedOffices[myhouseid]["trap"]) then
							...
							DrawText3Ds( curHouseCoords["x"]-1.4,curHouseCoords["y"]-1.47,curHouseCoords["z"]-32.2, '~p~'..Controlkey["housingMain"][2]..'~s~ to Browse' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerEvent("server-inventory-open", "1", PlayerStoreIdentifier)
								Citizen.Wait(5000)
							end
							DrawText3Ds( curHouseCoords["x"]-1.4,curHouseCoords["y"]-1.47,curHouseCoords["z"]-32.8, '~p~F~s~ to see reputation' )
				...
				elseif mansion2 then
						if (#(vector3(curHouseCoords["x"]+11.5,curHouseCoords["y"]+5.2,curHouseCoords["z"]-19.42) - playerCoords) < 1.2 ) then
							DrawText3Ds( curHouseCoords["x"]+11.5,curHouseCoords["y"]+5.2,curHouseCoords["z"]-19.42, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif mansion3 then
						if (#(vector3(curHouseCoords["x"] - 1.6,curHouseCoords["y"]+9.2,curHouseCoords["z"]-19.1) - playerCoords) < 1.2 ) then
							DrawText3Ds( curHouseCoords["x"] - 1.6,curHouseCoords["y"]+9.2,curHouseCoords["z"]-19.1, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif trailer then
					if (#(vector3((curHouseCoords["x"]-1.343),(curHouseCoords["y"]-2.292),(curHouseCoords["z"]-71.75)) - playerCoords) < 1.2 ) then
						DrawText3Ds( (curHouseCoords["x"]-1.343),(curHouseCoords["y"]-2.292),(curHouseCoords["z"]-71.75), '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
					...
					if apartment then
						if (#(vector3(curHouseCoords["x"] + 4.67,curHouseCoords["y"] - 6.56,curHouseCoords["z"] - 26.97) - playerCoords) < 1.2 ) then
							DrawText3Ds(curHouseCoords["x"] + 5.25,curHouseCoords["y"] - 6.76,curHouseCoords["z"] - 26.97, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave')
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif shithouse then
						if (#(vector3(curHouseCoords["x"] + 0.15,curHouseCoords["y"] - 9.71,curHouseCoords["z"] - 25.60) - playerCoords) < 1.2 ) then
							DrawText3Ds( curHouseCoords["x"] + 0.15,curHouseCoords["y"] - 9.71,curHouseCoords["z"] - 25.60, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif beachhouse then
						...
						if (#(vector3(curHouseCoords["x"],curHouseCoords["y"] - 4.3,curHouseCoords["z"]-31.08) - playerCoords) < 1.2 ) then
							DrawText3Ds( (curHouseCoords["x"]),(curHouseCoords["y"] - 4.3),(curHouseCoords["z"]-31.08), '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif IntegrityHouse then
						if (#(vector3(curHouseCoords["x"] + 2,curHouseCoords["y"] - 14.5,curHouseCoords["z"] - 26.7) - playerCoords) < 1.2) then
							DrawText3Ds(curHouseCoords["x"] + 2,curHouseCoords["y"] - 14.5,curHouseCoords["z"] - 26.7, '~p~'..Controlkey["housingMain"][2]..'~s~ to leave' ) 
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
...
robberycoordsMansions = {
	...
	[160] =  { ['x'] = -700.88,['y'] = 646.98,['z'] = 155.37,['h'] = 168.41, ['info'] = ' Hillcrest Avenue 1', ["office"] = 2 },
...
RegisterNetEvent('np-motels:RealtorPay')
AddEventHandler('np-motels:RealtorPay', function(amount)
	TriggerServerEvent("server:GroupPayment","real_estate", amount)
end)