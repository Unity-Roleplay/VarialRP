...
function hasSecuredAccess(pId, pType)
    ...
    for i=1, #mypasses do
        print(mypasses[i]["pass_type"])
        for k,v in pairs(secured.access.business) do
            print(k,v)
        end
        if secured.access.business and secured.access.business[mypasses[i]["pass_type"]] == true then
            accessCheckCache[pType][pId] = true
            return true
        end 
    end 
