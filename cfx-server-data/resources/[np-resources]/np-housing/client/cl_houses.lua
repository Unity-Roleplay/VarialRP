...
local SettingGarage = false
local SettingBed = false
...
function GetHouseInformation()
	...
	if housefound then
		...
		return houseinfo
	elseif not SettingGarage then
		TriggerEvent("DoLongHudText","No property found!",2)
	end
end
...
function GetNearestHouse()
	...
	local houseinfo = { [1] = house_id, [2] = house_model }
	for i=1,#robberycoords do
		if (#(vector3(robberycoords[i]["x"],robberycoords[i]["y"],robberycoords[i]["z"]) - GetEntityCoords(PlayerPedId())) < 15 ) then
			house_id = i
	...
	if house_id == 0 then
		...
		for i=1,#robberycoordsMansions do
			if (#(vector3(robberycoordsMansions[i]["x"],robberycoordsMansions[i]["y"],robberycoordsMansions[i]["z"]) - GetEntityCoords(PlayerPedId())) < 15 ) then
				house_id = i
				house_model = 2
				housefound = true
				curHouseCoords = robberycoordsMansions[i]
	...
	if house_id == 0 then
		...
		for i=1,#rentedOffices do
			if (#(vector3(rentedOffices[i]["location"]["x"],rentedOffices[i]["location"]["y"],rentedOffices[i]["location"]["z"]) - GetEntityCoords(PlayerPedId())) < 15 ) then
				house_id = rentedOffices[i]["house_id"]
				house_model = rentedOffices[i]["house_model"]
				housefound = true
...
local function getNearestproperty ()
	local house_model = 0
	local house_id = 0
	local housefound = false
	local houseinfo = { [1] = house_id, [2] = house_model }
...
RegisterNetEvent('np-housing:spawnpoints')
AddEventHandler('np-housing:spawnpoints', function(spawns)
...
RegisterCommand('housing-menu',function()
	-- you can set a real estate job here before opening menu
    TriggerEvent('np-context:sendMenu', {
        {
            id = 1,
            header = "Begin Modifying",
            txt = "Begin making changes",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "modify"
                }
            }
        ...
        },
        {
            id = 3,
            header = "Set Clothing",
            txt = "Set a wardrob to change clothes",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "setclothing",
        ...
        {
            id = 4,
            header = "Set Creationpoint",
            txt = "Set a creationpoint",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "creationpoint",
                }
            }
        },
		{
            id = 5,
            header = "Set Garage",
            txt = "Set house garage (must be outside !)",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "setgarage",
        ...
        },
		{
            id = 6,
            header = "Wipe Garages",
            txt = "Clear houses garage placement (remove vehicles first !)",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "wipegarages",
		...
		{
            id = 7,
            header = "Set Backdoor From Inside",
            txt = "Sets a door leading outside",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "backdoorinside",
                }
            }
        },
		...
		{
            id = 8,
            header = "Set Backdoor From Outside",
            txt = "Sets a door leading inside",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "backdoorinside",
		...
		{
            id = 8,
            header = "Set Wake Up Point",
            txt = "Sets a spawn point for your current home",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "setbedspawn",
		...
		{
            id = 9,
            header = "Attempt to pay morgage",
            txt = "pay morage on current property",
            params = {
                event = "housing:attemptpay",
            }
        },
...

		{
            ...
            txt = "Reset all current changes made ?",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "reset",
        ...
        },
		{
            id = 11,
            header = "Stop",
            txt = "When finished making changes or if cancelling ",
            params = {
                event = "housing:info:realtor",
                args = {
                    action = "stop",
...
RegisterNetEvent('np-housing:unlock&lockProperty', function()
...
RegisterNetEvent('housing:info:realtor') -- Some stuff checks if you are a real estate agent | aleks
AddEventHandler('housing:info:realtor', function(args)
	if modifying then
		...
		if args.action == "creationpoint" then
		...
		if args.action == "setclothing" then
			local coords = GetEntityCoords(PlayerPedId())
			local heading = GetEntityHeading(PlayerPedId())
		...
		if args.action == "setstorage" then
			local coords = GetEntityCoords(PlayerPedId())
			local heading = GetEntityHeading(PlayerPedId())
		...
		if args.action == "wipegarages" then
			local coords = GetEntityCoords(PlayerPedId())
			local heading = GetEntityHeading(PlayerPedId())
			modifyingTable["garages"] = {}
		...
		if args.action == "backdoorinside" then
			local coords = GetEntityCoords(PlayerPedId())
			local heading = GetEntityHeading(PlayerPedId())
		...
		if args.action == "backdooroutside" then
			local coords = GetEntityCoords(PlayerPedId())
			local heading = GetEntityHeading(PlayerPedId())
		...
		if args.action == "update" then
			TriggerEvent("DoLongHudText","Changes Updated !",1)
			TriggerServerEvent("np-housing:updatespawns",modifyingTable, modifyingTable["house_id"])
			return
		end
	end
	...
	if args.action == "unlock2" then
		TriggerServerEvent("housing:unlock",houseinfo[1],houseinfo[2])
	end

	if args.action == "PropertyOutstanding" then
		TriggerServerEvent("np-housing:PropertyOutstanding",houseinfo[1],houseinfo[2])
	end

	if args.action == "checklocks" then
		TriggerServerEvent("housing:unlockRE",houseinfo[1],houseinfo[2])
	end

	if args.action == "unlock" then
		TriggerServerEvent("housing:unlockRE",houseinfo[1],houseinfo[2])
...
AddEventHandler('housing:transferHouseAttempt', function(cid)
	if cid ~= nil then
		if modifying then
			TriggerServerEvent("np-housing:transferHouse",modifyingTable["house_id"],modifyingTable["house_model"],cid)
		else
...
AddEventHandler('housing:evictHouseAttempt', function()
	if modifying then
		TriggerServerEvent("np-housing:evictHouse",modifyingTable["house_id"],modifyingTable["house_model"],exports["isPed"]:isPed("cid"))
	else
...
Citizen.CreateThread(function()
	while true do
		...
		if backdooroutside["x"] ~= 0.0 and not insidehouse then
			...
			if #(vector3(backdooroutside["x"],backdooroutside["y"],backdooroutside["z"]-0.3) - plyCoords) < 1.5 then
				...
				if IsControlJustReleased(2, 38) then
					TriggerServerEvent("np-housing:enterhousebackdoor", house_poi["house_id"], house_poi["house_model"], backdoorinside["x"],backdoorinside["y"],backdoorinside["z"])
					Citizen.Wait(3000)
				end
			end	
		...
		if clothing["x"] ~= 0.0 and insidehouse then
			if #(vector3(clothing["x"],clothing["y"],clothing["z"]-0.3) - plyCoords) < 1.5 then
				DrawText3Ds( clothing["x"],clothing["y"],clothing["z"] , '[~g~'..Controlkey["housingSecondary"][2]..'~s~] - change characters | /outfits.' )
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,storage,clothing,garages)
	...
	if buy then
		TriggerEvent("DoLongHudText","Attempting purchase - please wait.",91)
			TriggerServerEvent("np-housing:purchasehouse",tonumber(checkcid),house_id,house_model,upfront,price,housename,storage,clothing,garages,false)
	end
...
AddEventHandler('housing:attemptpay', function()
	...
	TriggerServerEvent("np-housing:dopayment", house_id, house_model)
...
AddEventHandler('housing:enter', function()
	...
	if modifying then
		TriggerServerEvent("np-housing:enterhouse",exports['isPed']:isPed('cid'), house_id,house_model,true)
		housenameraid = house_name
	else
		TriggerServerEvent("np-housing:enterhouse",exports['isPed']:isPed('cid'), house_id,house_model,false)
		housenameraid = "none"
	end
...
RegisterNetEvent("np-housing:entersuccess")
AddEventHandler("np-housing:entersuccess", function(house_id,house_model,furniture, x,y,z)
	...
	if rentedOffices[tonumber(house_id)] then
		if (rentedOffices[tonumber(house_id)]["trap"] ~= nil) then
			DoScreenFadeOut(1)
			TriggerEvent("np-housing:entering")
		end
	end
...
RegisterCommand('dohousing', function(source , args)
	TriggerEvent('housing:info:realtor',args[1])
end)
...
RegisterCommand('buyhousebro',function()
	TriggerEvent('housing:sendPurchaseAttempt',exports['isPed']:isPed('cid'),100)
end)
...
AddEventHandler('houses:GiveKey', function()
	...
	if(distance ~= -1 and distance < 5) then
		TriggerServerEvent("np-housing:givekey",house_id,house_model,house_name,GetPlayerServerId(t))
...
AddEventHandler('houses:retrieveHouseKeys', function()
	...
	TriggerServerEvent("np-housing:retrieveKeys",house_id,house_model)	
end)
...
AddEventHandler('houses:removeHouseKey', function(target)
	...
	TriggerServerEvent("np-housing:removeKey",house_id, house_model, target)		
end)
...
Citizen.CreateThread(function()
	while true do 
		local houseinfo = GetNearestHouse()
		if houseinfo ~= nil then

			TriggerServerEvent("housing:getGarage", houseinfo[1],houseinfo[2])
		else
			--TriggerEvent("menu:housing", false)
			return
		end
		Citizen.Wait(5000)
    end
...
robberycoordsMansions = {
	...
	[261] =  { ['x'] = -143.76,['y'] = 229.72,['z'] = 94.94,['h'] = 178.75, ['info'] = ' Office Eclipse Blvd 9', ['office'] = 1 },
	[262] =  { ['x'] = -1356.6,['y'] = -791.17,['z'] = 20.25,['h'] = 311.43, ['info'] = ' Office Bay City Ave #1 ', ['office'] = 1 },
	[263] =  { ['x'] = 288.13,['y'] = -1095.02,['z'] = 29.42,['h'] = 85.8, ['info'] = 'Office Fantastic Place #1' , ['office'] = 1 },
	[264] =  { ['x'] = -930.32,['y'] = 19.47,['z'] = 48.52,['h'] = 35.60, ['info'] = ' Rockford Hils' ,['office'] = 2 },
...
exports('isNearProperty',function()
	local houseinfo = getNearestproperty()
	return houseinfo
end)
