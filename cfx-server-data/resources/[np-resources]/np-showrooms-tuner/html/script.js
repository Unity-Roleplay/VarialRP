...
var ShowroomFunctions = {
    ...
    GetCars: function() {
        $.post('https://np-showrooms-tuner/np-ui:tunershowroomChangeCar', JSON.stringify({ model: cars[selectedId].model }), function(cb) {
...
$(document).ready(function() {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.shop == 'pdm') {
            ...
            return
        } else if (data.shop == "tuner") {
            zone = "pdm"
            ShowroomFunctions.TUNER_Load()
            return
        }
        if (data.meta.message = "done") {
            ShowroomFunctions.Close()
            $.post('https://np-showrooms-tuner/np-ui:tunerapplication-closed', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
$(document).on("click", "#purchase", function() {
    canPress = false
    $.post('https://np-showrooms-tuner/tunershowroomPurchaseCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price: cars[selectedId].price, zoneName: zone }), function(cb) {
        if (cb.meta.ok) {
            ShowroomFunctions.Close()
            canPress = false
        } else {
