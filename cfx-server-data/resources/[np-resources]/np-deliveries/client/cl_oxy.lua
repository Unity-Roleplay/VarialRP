...
local bandprice = math.random(1200, 2200)
local markedbillsprice = math.random(500, 1500)
local rollcashprice = math.random(150, 800)
local rollcount = 3
local bandcount = 5
local markedbillsamount = math.random(2, 10)
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
            exports["np-ui"]:showInteraction("[E] Drop off")
			local crds = GetEntityCoords(deliveryPed)
			--DrawText3Ds(crds["x"],crds["y"],crds["z"], "[~r~E~w~ Drop Off]")  
...
Citizen.CreateThread(function()
	exports["np-polyzone"]:AddBoxZone("oxy_run_start", vector3(-1487.02, -909.79, 10.02), 1.8, 1, {
		name="oxy_run_start",
		heading=230,
		minZ=6.82,
		maxZ=10.82
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "oxy_run_start" then
        ...
        LuckyDrawPlace()
		if not OxyRun then
			exports['np-ui']:showInteraction('[E] Start Run 500$')
		else
			exports['np-ui']:hideInteraction()
		end
    end
...
function LuckyDrawPlace()
	Citizen.CreateThread(function()
        while OxySpot do
			...
			if IsControlJustReleased(0, 38) then
				if not OxyRun then
					TriggerServerEvent("oxydelivery:server", 500)
				end          
			end
		end
	end)
...
Citizen.CreateThread(function()
    while true do
        if drugdealer then
		    ...
		    if salecount == math.random(5, 10) then
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if OxyRun then
				...
				if exports["np-inventory"]:hasEnoughOfItem("rollcash",rollcount) then     
					...
					TriggerServerEvent('mission:finished', "rollcash", cashPayment)
				elseif exports["np-inventory"]:hasEnoughOfItem("band",bandcount) then     
					TriggerEvent("inventory:removeItem","band", bandcount)   
					cashPayment = cashPayment + bandprice              
					--TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
					TriggerServerEvent('mission:finished', "band", cashPayment)
				elseif exports["np-inventory"]:hasEnoughOfItem("markedbills",markedbillsamount) then     
					TriggerEvent("inventory:removeItem","markedbills", markedbillsamount)   
					cashPayment = cashPayment + markedbillsprice           
					--TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
					TriggerServerEvent('mission:finished', "markedbills", cashPayment)
				else
				...
				if math.random(100) == 20 then
					TriggerEvent( "player:receiveItem", "Gruppe6Card", math.random(1))
				end
		end
	end
	...
	if success then
		Citizen.Wait(2000)
		TriggerEvent("phone:addnotification", "EMAIL ", "Nice dropoff, Getting another deal setup for you.")
	else
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	print('Oxy Runs | Succesfuly Loaded')
	end)
