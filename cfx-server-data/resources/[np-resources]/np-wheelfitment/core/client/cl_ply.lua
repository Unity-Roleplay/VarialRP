...
function SyncWheelFitment()
    --if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            ...
            local vin = exports["np-garages"]:NearVehicle("sittingplate")
...
Citizen.CreateThread(function()
	...
	exports["np-polyzone"]:AddBoxZone("np-wheelfitment:zone2", vector3(135.88, -3030.43, 7.04), 6.4, 4.0, {heading = 0, minZ = 6.04, maxZ = 9.04, data = {id = "wheel_fitment_zone"}})
end)
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    if zone ~= "np-wheelfitment:zone2" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
...
AddEventHandler("np-polyzone:exit", function(zone)
    if zone ~= "np-wheelfitment:zone2" then
        return
    end
    inZone = false
end)
