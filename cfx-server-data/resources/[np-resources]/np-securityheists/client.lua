...
AddEventHandler('sec:AddPeds', function(veh)
    local cType = 'mp_m_securoguard_01'
...
function pickUpCash()
    ...
    if not pickingup then
        if math.random(10) == 1 then
            TriggerEvent( "player:receiveItem", "bobsecuritycard", 1 )
        end
       ...
       -- Citizen.Trace("Doing Animation")
        ...
        while pickingup do
            ...
            if chance > 65 and not gotcard then
                gotcard = true
                TriggerEvent("player:receiveItem", "bobsecuritycard", 1)
            end
            ...
            TriggerEvent("player:receiveItem","markedbills",math.random(length))
