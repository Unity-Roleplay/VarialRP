...
AddEventHandler('phone:sendSMS', function(cid, receiver, message)
  ...
  print('This is my cid', cid)
  print('Grabbing number poggers', mynumber)
...
AddEventHandler('racing-save-map', function(currentMap,name,description,distanceMap)
    ...
    exports.ghmattimysql:execute("INSERT INTO `racing_tracks` (`checkpoints`, `creator`, `track_names`, `distance`, `description`) VALUES ('"..json.encode(currentMap).."', '"..tostring(playername).."', '"..tostring(name).."', '"..distanceMap.."',  '"..description.."')", function(results)
        Wait(1000)
        buildMaps()
    end)
end)
...
AddEventHandler('account:information:sv', function(id)
    local src = source
    local licenceTable = {}
    exports.ghmattimysql:execute("SELECT licenses FROM characters WHERE id = @id",{['id'] = id}, function(result)
    ...
    exports.ghmattimysql:execute("SELECT `paycheck` FROM characters WHERE id = @id",{['id'] = id}, function(data)
        if (data[1]) then
  ...
  local cid = char.id
    exports.ghmattimysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
      if data[1].wallpaper ~= nil then
          ...
          TriggerClientEvent('np-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
          print('Not Grabbbing')
      end
    end)
end)