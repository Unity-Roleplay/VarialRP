...
function hasDecrypt()
  if exports["np-inventory"]:hasEnoughOfItem("vpnxj",1,false)  then
    return true
  else
    return false
  end
...
RegisterNUICallback('btnDecrypt', function()
  ...
  TriggerEvent("DoLongHudText","Connecting VPN",1)
  closeGui()
  TriggerEvent("shithappensg")
  else 
    TriggerEvent("DoLongHudText","You dont have a  VPN on you",2)
    return
  end
end)

-- RegisterCommand("Tweet", function(src, args, raw)
--   TriggerServerEvent('Tweet', "@Cody_Sharp", "Stop stealing np code", "07-12.3.1")
-- end)
...
RegisterNUICallback('racing:events:list', function()
  if (exports["np-inventory"]:hasEnoughOfItem("racingusb1", 1) or exports["np-inventory"]:hasEnoughOfItem("racingusb0", 1, true)) then
    if exports["np-inventory"]:hasEnoughOfItem("racingusb1", 1) then
        SendNUIMessage({
...
AddEventHandler("payphone:ui", function()
  local ph = exports["np-keyboard"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,
...
IRP = IRP or {}
IRP.DataControls = IRP.DataControls or {}

function IRP.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(IRP.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
IRP.Controls = IRP.Controls or {}
IRP.Controls.Current = {}
-- Current bind name and keys
IRP.Controls.Default = {
  ["tokoptt"] = "caps",
  ["loudSpeaker"] = "-",
  ["distanceChange"] = "g",
  ["tokoToggle"] = "leftctrl",
...
RegisterNUICallback('PostNewImage', function(data, cb)
  TriggerEvent("DoLongHudText", "[UP ARROW] SELFIE | [DOWN] CAMERA | [ENTER] CLICK")
  TriggerEvent('animation:sms', false)
  takePhoto = true
  Wait(500)
  while takePhoto do
