...
AddEventHandler("np-hospitalization:checkin", function(allowed)
	if allowed then
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["np-taskbar"]:taskBar(2500,"Checking Credentials")
...
AddEventHandler("np-hospitalization:page", function()
	if exports["isPed"]:isPed("countems") >= 1 then 
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["np-taskbar"]:taskBar(2500,"Paging a Doctor")
		if finished == 100 then
...
Citizen.CreateThread(function()
exports["np-polytarget"]:AddCircleZone("pillbox_checkin", vector3(307.59, -595.3, 43.08), 0.35, {
