...
AddEventHandler('store:robbery:register', function(storeid,RegisterObject)
    ...
    if storePedLocations[storeid].recent or #storePedLocations[storeid].registers >= 2 then 
		TriggerClientEvent('np-notification:client:Alert', src, {style = 'error', duration = 3000, message = 'This store has been robbed recently.'})
        return
    end
    
    for i = 1, #storePedLocations[storeid].registers do
        if storePedLocations[storeid].registers[i] == RegisterObject then
			TriggerClientEvent('np-notification:client:Alert', src, {style = 'error', duration = 3000, message = 'This register has been robbed already.'})
            return
        end
    end
