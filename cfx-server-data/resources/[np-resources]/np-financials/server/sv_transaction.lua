...
function prepareTransaction(pSource, pSourceAccountId, pTargetAccountId, pAmount, pComment, pTaxCategory, pTransactionType)
...
RPC.register("DoTransaction", prepareTransaction)
...
function getTransactions(pSourceAccountId, pTransactionType, pStartDate, pEndDate)
    local query = [[
        ...
        LEFT JOIN _account acc ON acc.id = a2.account_id
        LEFT JOIN _account acc2 ON acc2.id = a.account_id
...
RPC.register("BankDeposit", bankDeposit)
...
RPC.register("BankWithdraw", bankWithdraw)
