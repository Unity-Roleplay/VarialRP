-- GET TRANSACTIONS
RegisterNUICallback("np-ui:getAccountTransactions", function(data, cb)
...
RegisterNUICallback("np-ui:accountDeposit", function(data, cb)
...
RegisterNUICallback("np-ui:accountWithdraw", function(data, cb)
    local accountId, amount, comment = data.account_id, data.amount, data.comment
    local success, error = RPC.execute("BankWithdraw", accountId, tonumber(amount), comment)
    cb({ data = {}, meta = { ok = success, message = (not success and error or 'done') } })
end)

RegisterNUICallback("np-ui:accountTransfer", function(data, cb)
    ...
    if transferType and transferType == "venmo" then
      ...
      if targetStateId ~= nil and targetStateId ~= 0 then
