...
function financialAnimation(pIsATM, pIsOpening)
    ...
    if pIsATM then
        ...
        if pIsOpening then
            ...
            local finished = exports["np-taskbar"]:taskBar(3000, "Inserindo cartão")
            ...
            local finished = exports["np-taskbar"]:taskBar(1000, "Retirando cartão")
            ClearPedTasks(playerId)
        end
    else
        ...
        if pIsOpening then
            ...
            local finished = exports["np-taskbar"]:taskBar(1000, "Mostrando Documentos")
            ClearPedTasks(playerId)
        else
            TaskPlayAnim(playerId, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0)
            local finished = exports["np-taskbar"]:taskBar(1000, "Coletando documentação")
            Citizen.Wait(1000)
            ClearPedTasks(playerId)
        end
    end
