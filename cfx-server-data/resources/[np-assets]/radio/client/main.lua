...
function openGui()
  ...
  elseif job == "ems" then
    Emergency = true
  elseif exports["np-inventory"]:hasEnoughOfItem("scanner",1,false) then
    Emergency = true
  end
...
function hasRadio()
    if exports["np-inventory"]:hasEnoughOfItem("radio", 1, false, true) or exports["np-inventory"]:hasEnoughOfItem("civradio", 1, false, true)then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  TriggerEvent('np-voice:SetRadioVolume', radioVolume)
...
AddEventHandler('np-inventory:itemCheck', function (item, state, quantity)
    ...
    exports["np-voice"]:removePlayerFromRadio(false)
