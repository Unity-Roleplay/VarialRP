  ...
  if Config['General']["Core"] == "NPBASE" then
   if(tonumber(BNEBoosting['functions'].GetCurrentBNE().bne) >= tonumber(data.price)) then
    if exports[Config['CoreSettings']["NPBASE"]["HandlerScriptName"]]:isPed("countpolice") >= Config['General']["MinPolice"] then
      ...
      TriggerEvent("DoLongHudText", "Not enough BNE",'error')
     SetNuiFocus(false ,false)
	end
  end
end)
...
AddEventHandler("boosting:DisablerUsed" , function()
  if OnTheDropoffWay then
    ...
    if (Config['Utils']["Contracts"]["DisableTrackingOnDCB"]) and (Class == "D" or Class == "C" or Class == "B") then
      if Config['General']["Core"] == "NPBASE" then
          TriggerEvent("DoLongHudText", "Seems like this vehicle doesnt have a tracker on",2)
      end
    elseif(vinstarted == false) then
      if(DisablerTimes < 3) then
        DisablerUsed = true
        local minigame = exports['hackingminigame']:Open()   
        if(minigame == true and DisablerTimes < 3) then
          if(DisablerTimes == 2) then
    ...
    if(DisablerTimes < 4) then
      ...
      if(DisablerTimes == 4) then
        ...
        CanUseComputer = true
        if Config['General']["Core"] == "NPBASE" then
            TriggerEvent("DoLongHudText", "Tracker removed, head to the scratch location")
        end
    ...
    if Config['General']["Core"] == "NPBASE" then
        TriggerEvent("DoLongHudText", "No ongoing mission detected",2)
    end
  end
end
end)
...
AddEventHandler("boosting:DisplayUI" , function()
  ...
  if NuiLoaded then      
    ...
    if Config['General']["Core"] == "NPBASE" then
        TriggerEvent("DoLongHudText","Please wait 3 senconds the nui is loading",2)
    end
    Citizen.Wait(3000)
    NuiLoaded = true
  end
...
AddEventHandler("boosting:client:UseComputer" , function()
  if Config['General']["Core"] == "NPBASE" then
    if CanUseComputer then
      ...
      local finished = exports["np-taskbar"]:taskBar(14000, "Connecting to network..")
...
AddEventHandler("boosting:client:2ndUseComputer" , function()
  if Config['General']["Core"] == "NPBASE" then
    ...
    FreezeEntityPosition(PlayerPedId(),true)
    TaskPlayAnim(PlayerPedId(), ScratchAnimDict, ScratchAnim, 3.0, -8, -1, 63, 0, 0, 0, 0 )
    local finished = exports["np-taskbar"]:taskBar(4000, "Wiping online paperwork.")
    if finished == 100 then
      CanScratchVehicle = true
      TriggerEvent("DoLongHudText", "Head to the vehicle and scratch off the vin!")
...
AddEventHandler("boosting:client:ScratchVehicle" , function()
  if Config['General']["Core"] == "NPBASE" then
    ...
    if finished == 100 then
      AddVehicleToGarage()
      TriggerEvent("DoLongHudText", "Vehicle added to your garage!")
      CallingCops = false
      DeleteBlip()
      FreezeEntityPosition(PlayerPedId(),false)
      ClearPedTasks(PlayerPedId())
