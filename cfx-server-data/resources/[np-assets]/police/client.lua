...
AddEventHandler('police:getArrested2', function(cuffer)
    ...
    if not isDead and cuffAttemptCount < 4 then
        ...
        finished = exports["np-ui"]:taskBarSkill(cuffAttemptTbl[cuffAttemptCount][1],
...
RegisterCommand("evidence", function(source, args)
    ...
    if job == 'police' then
        ...
        TriggerEvent("DoLongHudText", "You're not a cop.", 2)
    end
end)
...
AddEventHandler('FlipVehicle', function()
    ...
    local finished = exports["np-taskbar"]:taskBar(18000, "Flipping Vehicle Over", false, true)
...
AddEventHandler("police:dorifto", function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if Driver == PlayerPedId() and IsVehicleModel(veh, defaultHash) or IsVehicleModel(veh, defaultHash2) or
            ...
            local finished = exports["np-taskbar"]:taskBar(3000, "Toggling Drift Mode", false, true)
...
AddEventHandler('police:frisk', function()
    ...
    if (distance ~= -1 and distance < 5) then
        ...
        local finished = exports["np-taskbar"]:taskBar(3000, "Frisking")
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)
