...
AddEventHandler('checkVehVin', function(plate)
    ...
    exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE `license_plate` = @plate', {['@plate'] = plate}, function(result)
        if result[1].vin == 1 then
            TriggerClientEvent("chatMessage", src, "DISPATCH", 3,"(Positive Vin Test) Plate: "..plate)
            TriggerClientEvent('InteractSound_CL:PlayOnOne', source, 'radioclick', 1.0)
        else
            TriggerClientEvent("chatMessage", src, "DISPATCH", 3,"(Negative Vin Test) Plate: "..plate)
            TriggerClientEvent('InteractSound_CL:PlayOnOne', source, 'radioclick', 1.0)
        end
    end)
end)
...
AddEventHandler('checkLicensePlate', function(oof)
    ...
    exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE `license_plate` = @plate', {['@plate'] = kekw}, function(result)
        ...
        elseif logged[#logged] ~= nil then
            ...
            TriggerClientEvent("chatMessage", source, "DISPATCH", 3,
                "10-74 (Negative) Name: " .. insert.firstname .. " " .. insert.lastname .. " Phone #: " ..
                    math.random(000, 999) .. '-' .. math.random(000, 999) .. '-' .. math.random(0000, 9999) ..
                    ' Job: Unemployed')
...
AddEventHandler('police:jailGranted', function(args)
    ...
    if not pdunit:getVar("job") == "police" then
        ...
        exports["np-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {
            target = playerName,
            reason = reason,
            cid = cid,
...
AddEventHandler("police:deletecrimes", function(target)
    ...
    if not pdunit:getVar("job" == "police") then
        ...
        exports["np-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {
            target = target,
            src = src
        })
        DropPlayer(src, "")
...
RegisterCommand('bill', function(source, args)
    ...
    exports.ghmattimysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
        if result[1].job == 'police' then
            ...
            TriggerClientEvent('DoLongHudText', tonumber(args[1]),
                'You have $' .. tonumber(user:getBalance()) .. ' in the bank!')
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["np-log"]:AddLog("HC Added Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["np-log"]:AddLog("HC Removed Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
...
function jailUser(player, reason, cid, time, src)
    ...
    if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
        ...
        exports["np-log"]:AddLog("Exploiter", jailer,
...
AddEventHandler("police:targetCheckWeapons", function(playerId,items)
    local src = source
    local char = exports["np-base"]:getModule("Player"):GetUser(playerId)
...
RegisterCommand('pstate', function(source, args)
    local src = source
    local users = exports["np-base"]:getModule("Player"):GetUser(src)
