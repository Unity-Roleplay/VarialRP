...
AddEventHandler("platecheck:frontradar", function()
    TriggerEvent('clientcheckLicensePlate',radarInfo.frontPlate)
end)

RegisterNetEvent("platecheck:rearradar")
AddEventHandler("platecheck:rearradar", function()
    TriggerEvent('clientcheckLicensePlate',radarInfo.rearPlate)
end)
    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetPedInVehicleSeat( vehicle, -1 ) == ped  ) then 
            ...
            if radarInfo.fwdVeh ~= nil and (oldFrontPlate == nil or oldFrontPlate ~= radarInfo.frontPlate) then
              ...
              if exports["np-vehicles"]:GetVehicleMetadata(radarInfo.fwdVeh, "isFlagged") or RPC.execute('np:vehicles:isFlagged', radarInfo.frontPlate) then
            ...
            if radarInfo.bwdVeh ~= nil and (oldRearPlate == nil or oldRearPlate ~= radarInfo.rearPlate) then
              ...
              if exports["np-vehicles"]:GetVehicleMetadata(radarInfo.bwdVeh, "isFlagged") or RPC.execute('np:vehicles:isFlagged', radarInfo.rearPlate) then
                TriggerEvent("radar:alarm")
